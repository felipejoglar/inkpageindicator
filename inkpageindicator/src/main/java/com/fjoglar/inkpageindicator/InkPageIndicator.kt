 ...
 *
 * @throws [NullViewpager2AdapterException].
 */
class InkPageIndicator @JvmOverloads constructor(
    ...
    fun setViewPager(viewPager: ViewPager2) {
        if (viewPager.adapter == null)
            throw NullViewpager2AdapterException()
            ...
            override fun onPageScrolled(
                ...
                if (isAttached) {
                    ...
                    if (currentPosition != position) {
                        ...
                        // adjust for that
                        if (fraction == 1f) {
                            leftDotPosition = currentPosition.coerceAtMost(position)
                        }
                    }
        ...
        } ?: run { throw NullViewpager2AdapterException() }
    ...
    private fun setCurrentPageImmediate() {
        ...
        dotCenterX?.let { selectedDotX = it[currentPage] }
    }
    ...
    private fun createMoveSelectedAnimator(
        ...
        val retreatAnimation = PendingRetreatAnimator(
            was, now, steps,
            if (now > was) RightwardStartPredicate(moveTo - (moveTo - selectedDotX) * 0.25f) else LeftwardStartPredicate(
        ...
        moveSelected.addUpdateListener { valueAnimator: ValueAnimator ->
    ...
    inner class PendingRetreatAnimator(
        ...
        init {
            ...
            val initialX1 =
                if (now > was) dotCenterX!![was].coerceAtMost(selectedDotX) - dotRadius else dotCenterX!![now] - dotRadius
            val finalX1 =
                if (now > was) dotCenterX!![now] - dotRadius else dotCenterX!![now] - dotRadius
            val initialX2 =
                if (now > was) dotCenterX!![now] + dotRadius else dotCenterX!![was].coerceAtLeast(
                    selectedDotX
                ) + dotRadius
            val finalX2 =
                if (now > was) dotCenterX!![now] + dotRadius else dotCenterX!![now] + dotRadius
